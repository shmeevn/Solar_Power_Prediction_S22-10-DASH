...
while True:
    ...
    xx.append(t)
    for idx in range(5):#loops are great........
        #xdates.append(dt.time(int(X[n][idx][1]), int(X[n][idx][2])))
        xdates.append("{:n}/{:n} {:n}:{:n}".format(X[n][idx][3], X[n][idx][4], X[n][idx][1], X[n][idx][2]))
    ...
    ax.plot(pred_y, 'r-', label="Prediction")
    ax.plot(true_y, 'g-', label="True")
    ax.set_xlabel('Time Step (15 minutes)')
    ax.set_ylabel('PV Demand (kW)')
    ax.legend()
    ax.set_xticks(range(len(xx)))
    ax.set_xticklabels(xdates, rotation=90)
    fig.savefig("pv_fig.png")
    plt.close(fig)
    
    if (str(sys.argv[2]) == 'weather'):
    
        fig, ax = plt.subplots(1, 1)
        ax.plot(temp_y, 'b-', label="Temperature")
        ax.set_xlabel('Time Step (15 minutes)')
        ...
        fig, ax = plt.subplots(1, 1)
        ax.plot(wind_y, 'b-', label="Wind")
        ax.set_xlabel('Time Step (15 minutes)')
        ax.set_ylabel('Wind (m/s)')
        ax.legend()
        ax.set_xticks(range(len(xx)))
        ax.set_xticklabels(xdates, rotation=90)
        fig.savefig("wind_fig.png")
        plt.close(fig)
        
        fig, ax = plt.subplots(1, 1)
        ax.plot(precip_y, 'b-', label="Precipitation")
        ax.set_xlabel('Time Step (15 minutes)')
        ax.set_ylabel('Precipitation (mm/hr)')
        ax.legend()
        ax.set_xticks(range(len(xx)))
        ax.set_xticklabels(xdates, rotation=90)
        fig.savefig("precip_fig.png")
        plt.close(fig)
        
        fig, ax = plt.subplots(1, 1)
        ax.plot(clouds_y, 'b-', label="Cloud Coverage")
        ax.set_xlabel('Time Step (15 minutes)')
        ax.set_ylabel('Cloud Coverage (%)')
        ax.legend()
        ax.set_xticks(range(len(xx)))
        ax.set_xticklabels(xdates, rotation=90)
        fig.savefig("clouds_fig.png")
        plt.close(fig)
