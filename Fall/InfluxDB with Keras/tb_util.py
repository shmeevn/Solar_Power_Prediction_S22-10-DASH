...
def loadData(filepath, seq_length, vsp, iFeatLst = None, oFeatLst = None):
    data = pd.read_csv(filepath, sep=';')#read file into dataframe
    if iFeatLst != None:#Resolve Column(feature) name -> index after drop
        iFeatLst = [data.columns.tolist().index(x) for x in iFeatLst if any([y==x for y in data.columns])]
    if oFeatLst != None:#Also resolve output
        oFeatLst = [data.columns.tolist().index(x) for x in oFeatLst if any([y==x for y in data.columns])]
    data = np.array(data)#make numpy array from dataframe (index-able)
    ...
    return X[:vs], Y[:vs], X[vs:], Y[vs:], X, Y
